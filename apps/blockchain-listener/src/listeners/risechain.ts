...
export class RisechainListener {
  ...
  private async handleTransfer(log: any) {
    ...
    // Publish deposit alert with telegramChatId so notification service can DM the user
    const chatId = (wallet as any).user?.telegramAccount?.chatId;
    if (chatId) {
      await this.publisher.publish(
