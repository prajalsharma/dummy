...
import { REDIS_CHANNELS, calculateUnrealizedPnl } from '@risex/shared';
  ...
  private async syncPnl() {
    ...
    for (const pos of positions) {
      ...
      if (tpHit || slHit) {
        ...
        logger.info('TP/SL triggered', { positionId: pos.id, triggerType, markPrice, triggerPrice });
    ...
    logger.debug('PnL sync complete', { positions: positions.length });
  }
